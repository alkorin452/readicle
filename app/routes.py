...
from app.forms import LoginForm, BookForm, EditBookForm
...
def addBook(shelf):
    ...
    if form.validate_on_submit():
        if shelf == "wishlist":
            book = Book(title=form.title.data, author=form.author.data, genre=form.genre.data, format=form.format.data, on_shelf=False)
        else:
            book = Book(title=form.title.data, author=form.author.data, genre=form.genre.data, format=form.format.data, on_shelf=True)
        db.session.add(book)
        db.session.commit()
        return redirect(url_for('index'))
...
@app.route('/edit/<shelf>', methods=['GET', 'POST'])
# @app.route('/edit')
@login_required
def edit(shelf):
    print(shelf)
...
@app.route('/delete/<shelf>/<int:bookid>', methods=['GET', 'POST'])
@login_required
def deleteBook(shelf, bookid):
    current_book = Book.query.get(bookid)
    if current_user.is_authenticated:
...
def wishlist():
    books = Book.query.filter_by(on_shelf=False).all()
    return render_template('index.html', title='Wishlist', books=books)
